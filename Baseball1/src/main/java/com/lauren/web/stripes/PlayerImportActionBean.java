        ...
        public Resolution doImport () {
            ...
            return new StreamingResolution("text/plain", "OK");
                ...
                private void insertPlayer(Session newSession, PlayerDTO player) {
                    ...
                    playerEntity.setHumanizedSalary(player.getHumanizedSalary());
                    ...
                    playerEntity.setSalary(player.getSalary());
                    ...
                    Criteria playLeague = newSession.createCriteria(League.class);
                    playLeague.add(Restrictions.eq("externalId", player.getLeagueId()));
                    League pLeague = (League) playLeague.uniqueResult();
                    playerEntity.setLeagueId(pLeague);

                    playerEntity.setPlayingPositionId(player.getPlayingPositionId());

                    Criteria playTeam = newSession.createCriteria(League.class);
                    playTeam.add(Restrictions.eq("externalId", player.getTeamId()));
                    Team pTeam = (Team) playTeam.uniqueResult();
